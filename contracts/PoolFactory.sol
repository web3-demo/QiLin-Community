...
contract PoolFactory is IPoolFactory, SystemSettings {
    ...
    constructor(address deployer01, address deployer02)
        SystemSettings(deployer02)
    {
        _deployer01 = deployer01;
    }

    function createPool(address oracleAddress_, address poolToken)
        external
        override
    {
        require(
            oracleAddress_ != address(0),
            "trade pair not found in uni swap"
        );
        require(
            pools[poolToken][oracleAddress_] == address(0),
            "pool already exists"
        );
        ...
        (address pool, address debt) = IDeployer01(_deployer01).deploy(
            poolToken,
            oracleAddress_,
            address(this),
            tradePair
